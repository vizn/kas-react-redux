...
//获取记账详情
export function getDetailData(className, year, month, accountId){
  return (dispatch, getState) => {
    ...
    if(!month){
      ...
      return fetch(Vizn.prototype.apiPath('detail/List'), {
            ...
            body: 'year='+ year+'&month='+month + '&size='+size+'&last_id=' + last_id + className + accountId
        }).then(response => response.json().then(json => ({ json, response })))
				  .then(({json,response}) => {
				  	if(!response.ok){
              SN.snalert('暂无数据')
...
账户互转action部分
*/
function receiveExid(val){
  return{
    type:EXCHANGE_ID,
    val: val
  }
}
function receiveFromacid(val) {
  return {
    type: EXCHANGE_FROMACID,
    val: val
  }
}
function receiveToacid(val) {
  return {
    type: EXCHANGE_TOACID,
    val: val
  }
}
function receiveExMoney(val) {
  return {
    type: EXCHANGE_MONEY,
    val: val
  }
}
function receiveFromacountdata(data){
  return{
    type: EXCHANGE_FROMACCOUNTDATA,
    data: data
  }
}
function receiveToaccountdata(data){
  return{
    type: EXCHANGE_TOACCOUNTDATA,
    data: data
  }
}
function receiveListdata(data){
  return{
    type: EXCHANGE_LISTDATA,
    data: data
  }
}
export function getExid(val){
  return dispatch => {
    dispatch(receiveExid(val))
  }
}
export function getFromacid(val){
  return dispatch => {
    dispatch(receiveFromacid(val))
  }
}
export function getToacid(val){
  return dispatch => {
    dispatch(receiveToacid(val))
  }
}
export function getExMoney(val){
  return dispatch => {
    dispatch(receiveExMoney(val))
  }
}
export function getFromaccountData(){
  return async (dispatch, getState) => {
    try{
      const response = await fetch(Vizn.prototype.apiPath('category/list'), {
        method: 'POST',
        credentials: 'include',
        headers: {
            'Authorization': `Bearer ${getCookie('token')}`,
      ...
      dispatch(receiveFromacountdata(json))
    }catch(err){
      console.log('异常错误：'+err.message)
    }
  }
}
export function getToaccountData(value){
  return async (dispatch, getState) => {
    let newData = []
    const json = getState().exchangeDate.toJS().fromaccountData
    for(let key in json){
      if(json[key]._id != value && key != value){
        newData.push(json[key])
      }
    }
    dispatch(receiveToaccountdata(newData))
  }
}
async function exchangeList(dispatch){
  try{
    const response = await fetch(Vizn.prototype.apiPath('exchange/list'), {
      method: 'POST',
      credentials: 'include',
      headers: {
          'Accept': 'application/json',
...
export function getExchangeList(){
  return (dispatch, getState) => {
    exchangeList(dispatch)
  }
}
export function updateExchange(){
  return async (dispatch, getState) => {
    try{
      Toa.loadPage('true', '正在提交')
      const response = await fetch(Vizn.prototype.apiPath('exchange/update'), {
        method: 'POST',
        credentials: 'include',
        headers: {
            'Accept': 'application/json',
        ...
        body: JSON.stringify(getState().exchangeDate.toJS().exchange)
      })
      const json = await response.json()
      Toa.loadPage('', '正在提交')
      if(json.success){
        SN.snalert('提交成功')
        dispatch(receiveFromacid(''))
        dispatch(receiveToacid(''))
        dispatch(receiveExMoney('0'))
        exchangeList(dispatch)
